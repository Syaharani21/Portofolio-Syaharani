...
function Band({
  ...
  const baseTexture = useTexture(lanyardImage || BLANK_PIXEL);
  // useTexture must be called unconditionally; use a blank pixel when an image
  // isn't supplied for a given face, then composite it below.
  const frontTex = useTexture(frontImage || BLANK_PIXEL);
  const backTex = useTexture(backImage || BLANK_PIXEL);
  const cardGeometry = useMemo(() => new THREE.BoxGeometry(1.7, 2.1, 0.04), []);
    ...
    <>
      <group position={[0, 4, 0]}>
        ...
        <RigidBody position={[2, 0, 0]} ref={card} {...segmentProps} type={dragged ? 'kinematicPosition' : 'dynamic'}>
          <CuboidCollider args={[0.85, 1.1, 0.02]} />
          <group
            scale={2.25}
            position={[0, -1.2, -0.05]}
